...
//			for (int n = 0; n < 10; n++) {
		...
		for(int i=0; i < meterArray.length(); i++){
			...
			String meterId = meterArray.getString(i);
			//TODO look for the meter with the id ending with 2
			if(meterId.substring(meterId.length()-1).equals("2")){
				...
				for(int j =0; j<metrics.length();j++){
					...
					if(currMetTxt.equals(metTxt)){
						...
						JSONArray intervals = intervals(responseMeas.asJson(), "array");
						boolean noMatchFound = true;
						//go through all intervals
						for(int h=0;h<intervals.length();h++){
							String valTime = intervals.getJSONArray(h).getString(0);
							String lookUpT = time;
							//look for then given time in the current interval
							if(valTime.equals(lookUpT)){
								//print value and information
								System.out.println("The avrage current is: ["+intervals.getJSONArray(h).getDouble(1)+"] for the time "+time+". For the Smart Meter with the ID: "+meterId);
								//match found
								noMatchFound = false;
								break;
							}
						}
						//print error if noMatchFound
						if(noMatchFound)
							System.out.println("There is no current it is the wrong time. Alternatives: "+intervals);
					}
				}
			}
		}
	...
	public static JSONArray intervals(HttpResponse<JsonNode> response, String path) {
		return (new JSONObject(response)).getJSONObject("body").getJSONArray(path);
	}
